...
// BinarizeSegmented запускает бинаризацию с оптимизированным порогом,
// гарантирует, что сегментированная облась является настолько большой и непрерывной, насколько это возможно
func BinarizeSegmented(in, out *matrix.M, meta types.Metadata) {
	binarize(in, out, math.Sqrt(meta.MeanValue))
}

// BinarizeSkeleton запускает бинаризацию с оптимизированным порогомб который
// не повреждает сам скелет
